...
| Команды, non-profit и юридические страницы | `src/pages/<page>/index.html`, `src/content/pages/<page>.json`; общая накидка редактора в `src/page/editor/` и `src/page/runtime/` |
| Наследие юридических страниц | `tools/site-pages/` хранит старые вспомогательные данные; canonical-источник редактируемых `/oferta/` и `/confpolicy/` теперь в `src/pages/` + `src/content/pages/` |
...
Google использует прежний inline-редактор: «править» → пароль 0281 → изменить текст → «сохранить». Google-аккаунт и список редакторов не требуются. `google-save-sync.yml` запускается сразу после сохранения: сервер создаёт служебный tag `editor-sync/<page>/<rev>-<sha>/<id>` на доверенном Main через отдельный deploy key только этого репозитория. Workflow читает пять Google-страниц, переносит разрешённые текстовые поля прямо в один коммит Wild и запускает release. Расписание раз в пять минут остаётся резервным опросом; своевременный запуск расписания GitHub не гарантируется. Редактор показывает Google-сохранение, подтверждённый коммит и публикацию отдельно; при сбое доступен повторный запуск. Ключ сервера находится вне Яндекс.Диска и в Secret Manager, в браузер не передаётся. Редактирование доступно на главной, `/ai-mindset-consulting/`, `/non-profit/`, `/oferta/` и `/confpolicy/`. Статусы привязаны к объекту, ревизии и hash сохранения; финальный статус требует совпадения source SHA на всех пяти Google-страницах и source/parent SHA в Pages. После сбоя доставки следующий запуск повторяет release того же коммита, не создавая дубль; активный выпуск не дублируется. PR не создаются. HTML/CSS/JS из Google не копируются в GitHub. Трёхстороннее сравнение и ревизии защищают от затёртых правок; меню, футер и обучение редактируются только в родителе через skill.

GitHub→Google работает через `tools/release/publish-google.py`, существующий `aim-v5-publisher` и объекты `wild/` существующего бакета. Доступ Actions короткоживущий (Workload Identity), постоянный Google-ключ не нужен. Старые файлы сохраняются в `wild/release-backups/`; журнал создаётся до записи. Неперенесённый Google-текст блокирует обратную публикацию. Обратное сохранение подтверждается короткоживущей подписью GitHub OIDC и не запускает новый импорт. Обычное сохранение браузера не может отключить этот запуск. Одинаковые версии не создают цикл коммитов.
...
Первое подключение вложенных редакторов закреплено точными проверенными hash в `tools/release/editor-bootstrap-20260914.json`; после появления source metadata действует обычная проверка предка и неперенесённых правок. Старый генератор `tools/site-pages/build.py` не имеет права перезаписывать migrated pages.
