...
(() => {
  ...
  const reserveTeamTextHeights = () => teamStates.forEach(state => {
    ...
    let tallest = natural;
    // Sample cyclic/reversed orders only on layout changes, never during animation.
    ...
    const lineHeight = parseFloat(getComputedStyle(state.text).lineHeight) || 0;
    // One extra wrapped line may use the padding; keep a 2px inset at the edge.
    const wrapAllowance = Math.max(0, lineHeight - bottomPadding + 2);
    state.text.style.height = Math.ceil(Math.max(natural + wrapAllowance, tallest - bottomPadding + 2)) + "px";
