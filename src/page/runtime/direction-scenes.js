...
(() => {
  ...
  function learningLogo(s,t){
    ...
    s.imprints.forEach(mark=>{mark.age+=.012;const fade=Math.max(.12,.88-mark.age*.05),x=mark.x*w,y=mark.y*h;c.strokeStyle=`rgba(197,13,23,${fade})`;c.lineWidth=1.5;c.beginPath();c.arc(x,y,7+mark.age*3,0,TAU);c.stroke()});s.imprints=s.imprints.filter(mark=>mark.age<14);
    s.trace.forEach(p=>p.a-=.018);while(s.trace[0]&&s.trace[0].a<=0)s.trace.shift();for(let i=1;i<s.trace.length;i++){const a=s.trace[i-1],b=s.trace[i];c.strokeStyle=`rgba(197,13,23,${b.a*.54})`;c.lineWidth=1.1+b.a*1.8;c.beginPath();c.moveTo(a.x*w,a.y*h);c.lineTo(b.x*w,b.y*h);c.stroke()}c.lineCap='butt';
  }
  function learningHead(s,t){
    ...
    s.imprints.forEach(mark=>{mark.age+=.012;const fade=Math.max(.12,.86-mark.age*.055),x=mark.x*w,y=mark.y*h;c.strokeStyle=`rgba(197,13,23,${fade})`;c.lineWidth=1.25;c.beginPath();c.arc(x,y,6+mark.age*3,0,TAU);c.stroke()});s.imprints=s.imprints.filter(mark=>mark.age<13);
    s.trace.forEach(p=>p.a-=.018);while(s.trace[0]&&s.trace[0].a<=0)s.trace.shift();for(let i=1;i<s.trace.length;i++){const a=s.trace[i-1],b=s.trace[i];c.strokeStyle=`rgba(197,13,23,${b.a*.82})`;c.lineWidth=1.2+b.a*2.15;c.beginPath();c.moveTo(a.x*w,a.y*h);c.lineTo(b.x*w,b.y*h);c.stroke()}
    const restArrowX=headX+headW*.08+4,restArrowY=headY-headH*.09-4,targetAngle=Math.atan2(my-h*.45,mx-w*.34),arrowX=mix(restArrowX,mx,active),arrowY=mix(restArrowY,my,active),restAngle=.063,arrowAngle=mix(restAngle,targetAngle,active);c.save();c.translate(arrowX,arrowY);c.rotate(arrowAngle);c.fillStyle='#C50D17';c.beginPath();c.moveTo(13,0);c.lineTo(-8,-4.7);c.lineTo(-8,4.7);c.closePath();c.fill();c.restore();c.lineCap='butt';c.lineJoin='miter';
  }
  function platform(s,t,dt){
    ...
    positions.forEach(({x,y,cell,i})=>{const color=i%7===0?'#C50D17':i%4===0?'#6B7280':'#0E8AA0',size=cell.size*(1+s.rings.length*.08);c.fillStyle=color;if(cell.kind===0)c.fillRect(x-size,y-size,size*2,size*2);else if(cell.kind===1){c.strokeStyle=color;c.lineWidth=1.3;c.strokeRect(x-size,y-size,size*2,size*2)}else{c.beginPath();c.arc(x,y,size,0,TAU);cell.kind===2?c.fill():c.stroke()}});
    s.rings.forEach(r=>r.age+=dt*.001);s.rings=s.rings.filter(r=>r.age<1.45);s.rings.forEach(r=>{const q=r.age/1.45;c.strokeStyle=`rgba(197,13,23,${(1-q)*.85})`;c.lineWidth=2;c.beginPath();c.arc(r.x*w,r.y*h,q*Math.min(w,h)*.62,0,TAU);c.stroke()});
  }
  function research(s,t,dt){
    ...
    closest.forEach((f,i)=>{
      ...
      c.strokeStyle=f.accent?'#C50D17':i%4===0?'#9E1727':'rgba(197,13,23,.9)';
      for(let n=1;n<=68;n++){
        const a=point(-1.07+(n-1)/68*2.14),b=point(-1.07+n/68*2.14),centre=ease(1-Math.min(1,Math.abs((a.x+b.x)*.5-mx)/r));
        if(centre>.18&&i%2===0){const dx=b.x-a.x,dy=b.y-a.y,len=Math.max(1,Math.hypot(dx,dy)),offset=(.38+centre*1.9)*(i%3+1)/3;c.strokeStyle='rgba(112,25,40,.52)';c.lineWidth=Math.max(.32,base*.34);for(const side of[-1,1]){const ox=-dy/len*offset*side,oy=dx/len*offset*side;c.beginPath();c.moveTo(a.x+ox,a.y+oy);c.lineTo(b.x+ox,b.y+oy);c.stroke()}c.strokeStyle=f.accent?'#C50D17':i%4===0?'#9E1727':'rgba(197,13,23,.9)'}
        c.lineWidth=base*(1+centre*3);c.beginPath();c.moveTo(a.x,a.y);c.lineTo(b.x,b.y);c.stroke();
      }
    });
    c.restore();
    s.discoveries.forEach(d=>{d.age+=dt*.001;const a=Math.max(0,1-d.age/11),x=d.x*w,y=d.y*h,rr=Math.min(w,h)*.075;if(!a)return;c.strokeStyle=`rgba(158,23,39,${a*.62})`;c.lineWidth=1.4;c.strokeRect(x-rr,y-rr,rr*2,rr*2);c.fillStyle=`rgba(197,13,23,${a*.8})`;c.fillRect(x-2,y-2,4,4)});s.discoveries=s.discoveries.filter(d=>d.age<11);
  ...
  function consulting(s,t){
    ...
    c.fillStyle='rgba(197,13,23,.18)';c.fillRect(carriedX-w*.085,carriedY+h*.056,w*.17,h*.045);c.fillStyle='#C50D17';c.fillRect(carriedX-w*.025,carriedY-w*.025,w*.05,w*.05);c.fillStyle='#f4f4ef';c.fillRect(carriedX-w*.008,carriedY-w*.008,w*.016,w*.016);
    if(s.pulse>0){s.pulse=Math.max(0,s.pulse-.006);const q=1-s.pulse;c.strokeStyle=`rgba(197,13,23,${s.pulse*.7})`;c.lineWidth=1.5;c.beginPath();c.arc(mx,my,w*(.045+q*.20),0,TAU);c.stroke()}c.lineCap='butt';c.lineJoin='miter';
  }
